...
public class UserPlayer {
    ...
    private RepeatStates isRepeating; // todo to use an enum here
    ...
    private void handleRepeatCase(int currentTimestamp) {
        ...
        } else if (!isPlayingPlaylist && isRepeating == RepeatStates.REPEAT_ONCE) {
    ...
    public void next(boolean isCommand, int currentTimestamp) {
        if (isRepeating == RepeatStates.REPEAT_CURRENT_SONG || isRepeating == RepeatStates.REPEAT_INFINITE) {
    ...
    void handleShuffle() {
        ...
        } else if (isShuffled && realIndex + 1 >= shuffledIndexes.size() && isRepeating == RepeatStates.REPEAT_ALL && isPlayingPlaylist) {
            realIndex = 0;
            playingIndex = shuffledIndexes.get(realIndex);
    ...
    public RepeatStates changeRepeatState() {
        ...
        } else if (isRepeating == RepeatStates.REPEAT_ONCE || isRepeating == RepeatStates.REPEAT_ALL) {
            if (isPlayingPlaylist) {
